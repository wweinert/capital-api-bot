...
// A full ten-pair minute archive is too large for the Raspberry Pi's default
// Node heap.  Keeping the universe explicit makes constrained runs
// reproducible and lets production research match the active live universe.
...
// Scoring-system control: its results must appear in the same report as MTF
// candidates instead of being silently replaced by a different signal engine.
const baseline = {
  method: "additive", threshold: 3, context: "m15", trigger: "any",
  trendMin: 0, setupMin: 0, wTrend: 1, wSetup: 1, wTrigger: 1,
  hold: 120, stopATR: 2.5, tpATR: 3, cooldown: 0,
  maxDaily: 99, maxPositions: 5, riskDivisor: 5,
};
...
results.push({ config: baseline, metrics: evaluate(prepared, baseline) });
...
const report = {
  ...
  baseline: results.find((result) => result.config === baseline),
  leaderboard: results.slice(0, 30),
};
